...
public class Gui implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cooldown.containsKey(player) && cooldown.get(player) > System.currentTimeMillis()) {
			...
			player.sendMessage(ChatColor.RED + "You must wait " + intRemaining + " seconds to execute this command");
	
		} else {

			cooldown.put(player, System.currentTimeMillis() + (10 * 1000));
			...
			ItemMeta exm = exit.getItemMeta();
			exm.setDisplayName(ChatColor.RED + "Exit");
			ArrayList<String> exl = new ArrayList<String>();
			exl.add("Exit the menu");
			exm.setLore(exl);
			exit.setItemMeta(exm);
